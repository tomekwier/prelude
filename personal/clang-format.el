...
(defun exec-unavailable-p (prog)
  "Predicate that checks if a given executable is available"
  (not (executable-find prog)))

(defun buffer-readonly-p (buffer)
  "Predicate that checks if a given buffer is writable"
...
(defun clang-format (begin end)
  ...
  (let* ((orig-windows (get-buffer-window-list (current-buffer)))
         ...
         (orig-point (point))
         (style "file"))
    (cond ((exec-unavailable-p clang-format-binary) (message "Cannot find clang-format, check your path"))
          ((buffer-readonly-p (current-buffer)) (message "Buffer is not writable"))
          ( t (unwind-protect (call-process-region 
                         (point-min) (point-max) clang-format-binary
                         t (list t nil) nil
